-- A marcação de ponto de causa volta a ser POR GERÊNCIA, e agora fica (§7.29).
--
-- Terceira e última ida ao mesmo lugar. O caminho, para quem ler isto depois:
--
--   §7.19  marcação por ÁREA          a área é a linha do quadro
--   §7.26  sobe para GERÊNCIA         "a marcação é geral" -- durou horas
--   §7.27  volta para ÁREA            "marca-se onde se conversa, lê-se onde
--                                      se prioriza" -- o Pareto dizia de qual
--                                      área a causa veio
--   §7.29  GERÊNCIA, definitivo       decisão do analista, 31/08/2026
--
-- O que decidiu: a área de venda é a camada do N5, que ainda não existe. Ela é
-- a linha do quadro e continua sendo -- os indicadores seguem por área --, mas
-- CONTAR a causa é ato da reunião do N4, e a reunião dele é uma só. Onze grades
-- para preencher numa reunião de 15 minutos era o que a tela pedia antes.
--
-- **O preço, dito por inteiro:** o Pareto deixa de dizer de qual área cada
-- causa veio. Era o que respondia "com qual supervisor eu falo". A conversa
-- continua acontecendo -- ela é que produz a contagem --, mas ela não fica mais
-- gravada. Quem quiser essa resposta de volta precisa do N5, não de um campo.
--
-- **E é por isso que ESTA direção é a segura**: subir SOMA, e soma é
-- bem-definida. Foi descer que não teve inverso em 29/08 e obrigou a apagar
-- duas linhas (`20260829140000`). Aqui nada se perde de quantidade -- só de
-- granularidade, e de propósito.
